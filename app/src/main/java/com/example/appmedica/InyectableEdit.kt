...
class InyectableEdit : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        dosis = intent.getStringExtra("cantidad") ?: "* Seleccione una opción"
        ...
        tipoInyeccion = intent.getStringExtra("lugarApl") ?: "* Seleccione una opción"
    ...
    private fun inicializaSpinners(){
        ...
        //valor por default capturado del extra
        val itemsDosis = resources.getStringArray(R.array.dosis_iny)
        val indexDosis = itemsDosis.indexOf(dosis)
        if (indexDosis != -1) {
            spinnercantidad.setSelection(indexDosis)
        }else{
            val dosisPersonalizada = Utilidades.extraerNumeros(dosis)
        ...
        //valor por default capturado del extra
        val itemsLugar = resources.getStringArray(R.array.tipos_inyectables)
        val indexLugar = itemsLugar.indexOf(tipoInyeccion)
        Log.i("MedEdit", "Tipo de inyeccion: $tipoInyeccion")
        if (indexLugar != -1) {
            spinnerTipoInyeccion.setSelection(indexLugar) // Seleccionar el índice encontrado
        }
